...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(32, 64, 5) # input = 110x110x32, output = 106x106x64

        # maxpool that uses a square window of kernel_size=2, stride=2
        self.pool2 = nn.MaxPool2d(2, 2) # output = input/2

        # 64 input image channels, 128 output channels/feature maps, 4x4 square convolution kernel
        self.conv3 = nn.Conv2d(64, 128, 4) # input = 53x53x64, output = 50x50x128

        # maxpool that uses a square window of kernel_size=2, stride=2
        self.pool3 = nn.MaxPool2d(2, 2) # output = input/2

        # fully connected layer with 640 output values
        self.fc1 = nn.Linear(25*25*128, 640)

        # dropout with p=0.2
        self.drop_fc1 = nn.Dropout(p=0.2)

        # fully connected layer with 136 output values (68 coordinates/keypoints)
        self.fc2 = nn.Linear(640, 136)
